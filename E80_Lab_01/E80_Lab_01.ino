...
void loop() {
  ...
  if (currentTime < 45000) {
  ...
  else if (currentTime > 45000 && currentTime < 48000) {
    // S1: Dive for 3 seconds
    // Note: if in testing robot goes up, flip sign for motor C
    motorDriver.drive(0, 0, -200); 
  }

   else if (currentTime > 48000 && currentTime < 52000) {
    // S2: Forward for 4 seconds
    ...
    motorDriver.drive(150, 200, 0); 
  } 

  else if (currentTime > 56000 && currentTime < 66000) {
    // S3: Move robot to surface for 10 seconds, will just power past surface if too fast
